...
const int N = 3799;//矩阵列数
const int r = 2759;//消元子数
const int e = 1953;//被消元行数
int R[N][N] = { 0 };//消元子
int E[N][N] = { 0 };//被消元行
int txt_r[N][N];
int txt_e[N][N];
...
int main()
{
	...
	//Groebner_ordinary(R, E, e);
	//Groebner_SIMD_SSE(R, E, e);
	//Groebner_SIMD_AVX(R, E, e);
	//Groebner_omp(R, E, e);
	//Groebner_omp_SSE(R, E, e);
	Groebner_omp_AVX(R, E, e);
	QueryPerformanceCounter((LARGE_INTEGER*)&tail);
	time = tail - head;
	cout << "Time: " << (time * 1000.0 / freq) / 1.0 << "ms" << endl;
